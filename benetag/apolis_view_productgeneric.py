    ...
    def get(self):
        ...
        if generic.isLine:
            if generic.numConfigs() > 1:
                self.redirect('/apolis_viewproduct?%s' % urllib.urlencode({'id': generic.key()}))
                return
            else:
                _configs = generic.getConfigs()
                for _config in _configs:
                    if _config:
                        self.redirect('/apolis_viewconfig?%s' % urllib.urlencode({'id': _config.key()}))
                        return
                return
                        
        if generic.isConfig:
            self.redirect('/apolis_viewconfig?%s' % urllib.urlencode({'id': generic.key()}))
            return
        if generic.isUnit:
            self.redirect('/apolis_viewunit?%s' % urllib.urlencode({'id': generic.key()}))
            return
